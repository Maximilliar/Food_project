...
function calc() {
    ...
    function calcTotal() {
        if (!sex || !height || !weight || !age || !ratio) {
            result.textContent = '** Заполните поля ввода';
            return;
        }
        ...
        result.style.fontSize = 32 + 'px';
        result.style.marginRight = 60 + 'px';
...
function form(formSelector, modalTimerId) {
    ...
    const message = {
        ...
        success: 'Спасибо! <br> <br>Скоро мы с вами свяжемся',
        failure: 'Error'
    };
...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    const slides = document.querySelectorAll(slide),
        ...
        slidesField = document.querySelector('.offer_slider-inner'); // Error если стаивть просто field
    const dots = [];
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            offset = Number.parseInt(width) * (slideTo - 1);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2021-09-28');
